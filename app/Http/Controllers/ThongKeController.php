...
class ThongKeController extends Controller
{
    ...
    {
        if(!Auth::check() || (Auth::check() && Auth::user()->VaiTro != 1))
        ...
        if ($request->input('kieuthongke') === 'sokhach') {
            ...
            // $soluong = 10;
        } else if ($request->input('kieuthongke') === 'doanhthu') {
            $soluong = ThongKe::thongKeDoanhThuByDay($start_day, $end_day);
        }


        return view('ThongKe.index', compact('soluong', 'start_day', 'end_day', 'request'));
    }
