...
<div class="col-8 col-md-10">
    ...
    <form action="{{route('tintucs.store')}}" method="POST" class="d-grid gap-3 w-md-50 mx-auto" enctype="multipart/form-data">
        @csrf
        <div class="d-flex flex-column gap-2">
            <label for="TenSuKien">Tên sự kiện*</label>
            <input value="{{ old('TenSuKien') }}" type="text" id="ten-su-kien" name="TenSuKien" placeholder="Nhập tên sự kiện" class="py-1 px-2 rounded border border-1">
            @error('TenSuKien')
        ...
        <div class="row">
            <div class="d-flex flex-column gap-2 col-6">
                ...
                <input value="{{ old('NgayDang') }}" type="datetime-local" id="ngay-dang" name="NgayDang" class="py-1 px-2 rounded border border-1">
                @error('NgayDang')
                <div class="text-danger fw-bold">{{$message}}</div>
                @enderror
            </div>
        ...
        <div class="d-flex flex-column gap-2">
            ...
            <textarea name="NoiDung" id="noi-dung" rows="10" placeholder="Nhập nội dung tin tức" class="py-1 px-2 rounded border border-1">{{ old('NoiDung') }}</textarea>
            @error('NoiDung')
            <div class="text-danger fw-bold">{{$message}}</div>
            @enderror
        </div>
