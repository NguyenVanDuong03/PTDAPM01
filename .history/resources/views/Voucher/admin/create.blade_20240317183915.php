...
<div class="col-8 col-md-10">
    ...
    <form action="{{route('vouchers.store')}}" method="POST" class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        ...
        <div class="row">
            <div class="d-flex flex-column gap-2 col-12 col-sm-6">
                ...
                <input value="{{ old('TiLeChietKhau') }}" type="number" id="ti-le-chiet-khau" name="TiLeChietKhau" class="py-1 px-2 rounded border border-1">
                @error('TiLeChietKhau')
                <div class="text-danger fw-bold">{{$message}}</div>
                @enderror
            </div>
            <div class="d-flex flex-column gap-2 col-12 col-sm-6">
                ...
                <input value="{{ old('HanMuc') }}" type="number" id="han-muc" name="HanMuc" class="py-1 px-2 rounded border border-1">
                @error('HanMuc')
                <div class="text-danger fw-bold">{{$message}}</div>
                @enderror
            </div>
        ...
        <div class="row">
            <div class="d-flex flex-column gap-2 col-12 col-sm-6">
                ...
                <input value="{{ old('NgayTao') }}" type="date" name="NgayTao" id="ngay-tao" class="py-1 px-2 rounded border border-1">
                @error('NgayTao')
                <div class="text-danger fw-bold">{{$message}}</div>
                @enderror
            </div>
            <div class="d-flex flex-column gap-2 col-12 col-sm-6">
                ...
                <input value="{{ old('HanDung') }}" type="datetime-local" name="HanDung" id="han-dung" class="py-1 px-2 rounded border border-1">
                @error('HanDung')
                <div class="text-danger fw-bold">{{$message}}</div>
                @enderror
            </div>
