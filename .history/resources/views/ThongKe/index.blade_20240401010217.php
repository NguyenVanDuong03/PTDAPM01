            ...
            <form id="findInfoWithDate" action="{{route('thongkes.index')}}" method="get">
                <div class="d-flex gap-3">
                    ...
                    <select name="kieuthongke" id="kieuthongke" class="w-25 border rounded-2">
                        <option value="doanhthu" @if($request->input('kieuthongke') == 'doanhthu') selected @endif>Doanh thu</option>
                        <option value="sokhach" @if($request->input('kieuthongke') == 'sokhach') selected @endif>Số khách</option>
                        <option value="tylebanve" @if($request->input('kieuthongke') == 'tylebanve') selected @endif>Tỷ lệ bán vé</option>
                    </select>
