...
class VoucherController extends Controller
{
    ...
    {
        $vouchers = Voucher::all()->sortByDesc('MaVoucher');
        if (!Auth::check()) {
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'TinhTrang' => ['required'],
            'HanDung' => ['required', 'after_or_equal:NgayTao'],
        ], [
            'TiLeChietKhau.required' => 'Tỷ lệ chiết khấu không được bỏ trống',
            'TiLeChietKhau.numeric' => 'Tỷ lệ chiết khấu không được chứa ký tự hoặc ký tự đặc biệt',
            ...
            'TiLeChietKhau.max' => 'Tỷ lệ chiết khấu không được lớn hơn 0.5',
        ...
        // if (NhanVien::where('TenDangNhapNV', $request->TenDangNhapNV)->exists()) {
        //     $validator->errors()->add('TenDangNhapNV', 'Tên đăng nhập nhân viên không tồn tại');
