...
<div class="col-8 col-md-10">
    ...
    <h3>Sửa thông tin loại đồ ăn</h3>
    <form method="POST" action="{{route('loaidoans.update', ['loaidoan'=>$loaidoan->MaTheLoai])}}" class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        @method('PUT')
        <div class="flex-column d-flex gap-2">
            <label for="TenTheLoai" class="">Tên loại đồ ăn*</label>
            <input name="TenTheLoai" value="{{$loaidoan->TenTheLoai}}" type="text" id="ten-loai-do-an" class="py-1 px-2 rounded border border-1" placeholder="Nhập tên loại đồ ăn">
            @error('TenTheLoai')
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <textarea name="MoTa" id="mo-ta" cols="30" rows="10" class="px-2 py-1 rounded-2" placeholder="Nhập mô tả">
            {{$loaidoan->MoTa}}
            </textarea>
            @error('MoTa')
            <div class="text-danger fw-bold">{{$message}}</div>
