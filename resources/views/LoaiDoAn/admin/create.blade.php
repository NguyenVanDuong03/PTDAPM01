...
<div class="col-8 col-md-10">
    ...
    <form action="{{route('loaidoans.store')}}" method="POST" class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        <div class="flex-column d-flex gap-2">
            <label for="TenTheLoai" class="">Tên loại đồ ăn*</label>
            <input value="{{ old('TenTheLoai') }}" name="TenTheLoai" type="text" id="ten-loai-do-an" class="py-1 px-2 rounded border border-1" placeholder="Nhập tên loại đồ ăn">
            @error('TenTheLoai')
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <textarea name="MoTa" id="mo-ta" cols="30" rows="10" class="px-2 py-1 rounded-2" placeholder="Nhập mô tả">{{ old('MoTa') }}</textarea>
            @error('MoTa')
            <div class="text-danger fw-bold">{{$message}}</div>
            @enderror
        </div>
