...
<div class="col-8 col-md-10">
    ...
    <h3>Chỉnh sửa thể loại phim</h3>
    <form method="POST" action="{{ route('theloais.update',$theloai->MaTheLoai) }}"
        class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        @method('PUT')
        <div class="flex-column d-flex gap-2">
            <label for="ten_nha_cung_cap" class="">Tên thể loại phim*</label>
            <input value="{{ $theloai->TenTheLoai }}" type="text" maxlength="50" id="TenTheLoai" name="TenTheLoai"
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <input value="{{ $theloai->MoTa }}" type="text" id="MoTa" name="MoTa" class="py-1 px-2
            rounded border border-1" placeholder="Nhập mô tả">
        </div>
