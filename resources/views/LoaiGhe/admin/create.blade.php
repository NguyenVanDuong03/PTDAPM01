...
<div class="col-8 col-md-10">
    ...
    <form action="{{route('loaighes.store')}}" method="POST" class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        <div class="flex-column d-flex gap-2">
            <label for="ten-loai-ghe" class="">Tên loại ghế*</label>
            <input type="text" id="ten-loai-ghe" maxlength="100" class="py-1 px-2 rounded border border-1"
                name="TenLoaiGhe" placeholder="Nhập tên loại ghế">
            @error('TenLoaiGhe')
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <input type="number" id="gia-ve" min="0" max="1000000" class="py-1 px-2 rounded border border-1"
                name="GiaVe" placeholder="Nhập giá vé">
            @error('GiaVe')
            <div class="text-danger fw-bold">{{$message}}</div>
            @enderror
        </div>
