...
<div class="col-8 col-md-10">
    ...
    <form action="{{route('ghengois.update',$ghengoi->MaGhe)}}" method="POST" class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        @method('PUT')
        <div class="flex-column d-flex gap-2">
            <label for="ma-phong">Phòng chiếu*</label>
            <select class="form-select form-select-sm" name="MaPhong" aria-label="Small select example">
                ...
                <option value="{{ $phong->MaPhong }}" {{ $phong->MaPhong == $ghengoi->MaPhong ?
                    'selected' :
                    '' }}>
                    {{ $phong->TenPhong }}
                </option>
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <select class="form-select form-select-sm" name="MaLoaiGhe" aria-label="Small select example">
                ...
                <option value="{{ $loaighe->MaLoaiGhe }}" {{ $loaighe->MaLoaiGhe == $ghengoi->MaLoaiGhe ? 'selected' :
                    '' }}>{{ $loaighe->TenLoaiGhe }}</option>
                @endforeach
            </select>
        </div>
        <div class="flex-column d-flex gap-2">
            <label for="vi-tri-day" class="">Vị trí dãy*</label>
            <input value="{{ $ghengoi->ViTriDay }}" maxlength="2" type="text" id="vi-tri-day"
                class="py-1 px-2 rounded border border-1" name="ViTriDay" placeholder="Nhập vị trí dãy">
            @error('ViTriDay')
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <input value="{{ $ghengoi->ViTriCot }}" type="number" min="0" id="vi-tri-cot"
                class="py-1 px-2 rounded border border-1" name="ViTriCot" placeholder="Nhập vị trí cột">
            @error('ViTriCot')
            <div class="text-danger fw-bold">{{$message}}</div>
            @enderror
        </div>
