...
@section('main')
    <main class="p-2 pt-5 d-flex flex-column justify-content-between">
        <div class="">
                            ...
                            @foreach ($Ghes as $row)
                                @foreach ($row as $element)
                                    @if ($element->TrangThai == 0)
                                        <button type="button" disabled class="btn-ghe col-2 btn-danger"
                                            onclick="toggleColor(this, {{ $i }}, {{ $element->GiaVe }})">{{ $Key[$element->ViTriDay - 1] }}{{ $element->ViTriCot }}</button>
                                        <input type="hidden" name="MaGhe[]" id="input0{{ $i }}" value="x">
                                        <input type="hidden" name="" id="input1{{ $i }}"
                                            value="{{ $element->MaGhe }}">
                                    @else
                                        <button type="button" class="btn-ghe col-2"
                                            onclick="toggleColor(this, {{ $i }}, {{ $element->GiaVe }})">{{ $Key[$element->ViTriDay - 1] }}{{ $element->ViTriCot }}</button>
                                        <input type="hidden" name="MaGhe[]" id="input0{{ $i }}" value="x">
                                        <input type="hidden" name="" id="input1{{ $i }}"
                                            value="{{ $element->MaGhe }}">
        ...
        <div class="d-flex gap-2 align-items-center justify-content-between rounded-2 mt-5 text-white px-3 py-2"
            ...
            <div class=" d-flex gap-3">
                <img src="{{asset("$Phim->DuongDan")}}" style="max-width: 120px" alt="" class="rounded">
                <div class="d-flex align-items-center">
                    <h5 class ="d-flex flex-column gap-2">
