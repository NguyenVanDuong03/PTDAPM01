...
<div class="col-8 col-md-10">
    ...
    <h3>Sửa quy định</h3>
    <form method="POST" action="{{route('quydinhs.update', $quydinh->MaQuyDinh)}}" class="d-grid gap-3 w-md-50 mx-auto">
        @csrf
        @method('PUT')
        <div class="flex-column d-flex gap-2">
            <label for="TieuDe" class="">Tiêu đề*</label>
            <input name="TieuDe" maxlength="250" value="{{$quydinh->TieuDe}}" type="text" id="ten-loai-do-an" class="py-1 px-2 rounded border border-1" placeholder="Nhập tiêu đề">
            @error('TieuDe')
        ...
        <div class="flex-column d-flex gap-2">
            ...
            <textarea name="NoiDung"  maxlength="1500" id="mo-ta" cols="30" rows="10" class="px-2 py-1 rounded-2" placeholder="Nhập nội dung">
            {{$quydinh->NoiDung}}
            </textarea>
            @error('NoiDung')
            <div class="text-danger fw-bold">{{$message}}</div>
